namespace NuclearReMind
{
    ...
    /// เชื่อมหลายเงื่อนไขแบบ "อย่างใดอย่างหนึ่ง" ด้วย '|' เช่น "heat_above_80|q_above_0.3"
    /// เงื่อนไขที่ไม่รู้จัก = false (ปลอดภัยกว่ายิงมั่ว)
    /// </summary>
    public static class StatCondition
    {
        /// <summary>ประเมินเงื่อนไข ณ จบวัน (day) กับ snapshot ทรัพยากร/เตาที่ caller แคชไว้จาก event</summary>
        public static bool Matches(string condition, int day, ResourceData resources, TowerData tower)
        {
            ...
            if (condition.IndexOf('|') >= 0)
            {
                foreach (var part in condition.Split('|'))
                    if (Matches(part, day, resources, tower)) return true;
                return false;
            }
