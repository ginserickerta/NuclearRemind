...
|---|---|---|---|
| Healthy | — | 100% | 0 |
| Tired | fatigue > 45 ⚠ | 100% | 0 |
| Exhausted | fatigue > 68 ⚠ | 70% | −1 |
| **Hungry** | **hunger > 55** | 60% | −2 |
| Sick | radiation > 50 | **0%** | −3 |
| Dying | radiation > 80 | **0%** | −5 |
| Dead | — | — | −8 ครั้งเดียว |

> ⚠ **ป้ายสถานะกับประสิทธิภาพใช้เลขคนละชุด — ตั้งใจให้แยกกัน**
> ป้ายมีไว้ให้ผู้เล่นอ่าน ประสิทธิภาพมีไว้ให้ระบบคำนวณ สองอย่างนี้ไม่จำเป็นต้องใช้เส้นเดียวกัน
>
> | | ป้ายสถานะ | ประสิทธิภาพ |
> |---|---|---|
> | ล้า | `tired_threshold` = 45 | `eff_fatigue_soft` = 60 → ×0.7 |
> | หมดแรง | `exhausted_threshold` = 68 | `eff_fatigue_hard` = 85 → 0% |
>
> **ทำไม Exhausted ต้องเป็น 68 ไม่ใช่ 85**
> เดิมสองอย่างนี้เป็นเลขตัวเดียวกัน แต่ `ShiftSystem` ดึงคนไปพักที่ 70 คนงานจึงมียอดความล้าสูงสุด
> แค่ **72** และไม่มีวันแตะ 85 → สถานะนี้ไม่เคยถูกติดให้ใครเลยตลอดเกม และการ์ด 5 `overwork`
> (`exhaustedWorkers >= 3`) ก็ไม่มีทางเกิด แยกเลขออกเป็นสองตัวจึงทำให้ทั้งสองอย่างทำงานตามที่เขียนไว้
>
> **ทำไม Tired ต้องเป็น 45 ไม่ใช่ 60**
> ความล้าเดินทีละ 12 (12→24→36→48→60→72) ถ้าใช้ 60 ตั้งชื่อ ช่วง "ล้า" จะเหลือแค่ 61-68 ซึ่ง
> **ไม่มีค่าไหนตกลงไปเลย** ผู้เล่นจะเห็น "ปกติ" ติดกัน 5 วันแล้วจู่ ๆ เป็น "หมดแรง" ไม่มีสัญญาณเตือน
> ล่วงหน้า · 45 ทำให้ 48 กับ 60 ติดป้าย "ล้า" = เตือน 2 วันก่อนถูกดึงไปพัก และ 42 (วันแรกหลังพัก)
> กลับเป็น "ปกติ" อย่างมีความหมาย
>
> **ผลผลิตไม่เปลี่ยนแม้แต่หน่วยเดียว** — คอลัมน์ Eff ข้างบนคิดจาก 60/85 เหมือนเดิมทุกประการ และ
> คนงานไม่เคยผลิตที่ความล้าเกิน 60 อยู่แล้ว (ผลิตเกิดก่อนความล้าเพิ่ม + ระบบพักดึงออกที่ 70 เสมอ)
> คนที่ป้ายว่า "หมดแรง" (ล้า 72) จึงยังทำงานได้ 70% ตามตาราง แล้วถูกดึงไปพักในวันเดียวกันนั้น
